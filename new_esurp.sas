...
/*   1. retrieve CRSP cumulative adjustment factor for IBES report and estimate dates */
		create table adjfactor
                ...
                from dsf (keep=permno date cfacshr) as a, ibes1 as b
...
proc sql; create table lnk1 (drop=permno score where=(missing(ticker)=0))
        ...
        home.iclink (keep=ticker permno score where=(score in (0,1,2))) as b
...
proc sql;
   ...
   from dsf (keep=permno ret date where=(&begindate<=date<=&enddate)) as a,
...
proc sort data=suecars out=home.suecars; by gvkey fyearq fqtr;run;
